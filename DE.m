function A_bars = DE(A_bars,w_tar,A_conv,DEparams)
    ...
    for p = 1:P
        ...
        score(p) = sum(abs(w(:,p)-w_tars(:,p))); % score to asses how close one is to the target 
    end
    % Step 2
    [~, p_star] = min(score);
    % Step 3
    for p = 1:P
        ...
        if abs(w_primes(:,p)-w_tars(:,p)) < abs(w(:,p)-w_tars(:,p)) % if w'(p) are closer to the target, we return A'(p), else A(p)
            A_bars(:,:,p) = A_primes(:,:,p);
        end
    end
end